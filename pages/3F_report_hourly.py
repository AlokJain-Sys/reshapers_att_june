...
# Format Date, In_Time, and Out_Time columns for shift_wise_report
...
# Create new combined hourly report
hourly_combined_report = filtered_df.groupby(['Date', 'Shift', 'Name', pd.Grouper(key='Timestamp', freq='H')]).agg(
    In_Time=('Timestamp', 'min'),
    Out_Time=('Timestamp', 'max')
).reset_index()

# Calculate Duration
hourly_combined_report['Duration'] = hourly_combined_report['Out_Time'] - hourly_combined_report['In_Time']

# Format Date, In_Time, and Out_Time columns for hourly_combined_report
hourly_combined_report['Date'] = pd.to_datetime(hourly_combined_report['Date']).dt.strftime('%d-%b-%Y')
hourly_combined_report['In_Time'] = hourly_combined_report['In_Time'].dt.strftime('%I:%M %p')
hourly_combined_report['Out_Time'] = hourly_combined_report['Out_Time'].dt.strftime('%I:%M %p')

# Format Duration as HH:MM:SS
hourly_combined_report['Duration'] = hourly_combined_report['Duration'].apply(lambda x: f"{x.components.hours:02d}:{x.components.minutes:02d}:{x.components.seconds:02d}")

# Reorder columns
hourly_combined_report = hourly_combined_report[['Date', 'Shift', 'Name', 'In_Time', 'Out_Time', 'Duration']]
...
# Format Date, In_Time, and Out_Time columns for date_name_role_zip_df
...
if not date_name_role_zip_df.empty:
    ...
    st.write("## Shift-Wise Summary:")
    st.dataframe(shift_wise_report)

    st.write("## Hourly Combined Report:")
    st.dataframe(hourly_combined_report)
